document.addEventListener("DOMContentLoaded", () => {
  ...
  function type() {
    ...
    textElement.innerHTML = current.substring(0, j) + '<span class="cursor">|</span>';
    ...
    if (!ticking) {
      requestAnimationFrame(() => {
        ...
        if (scroll <= height) {
          ...
          if (knight) knight.style.transform = `translate(${-scroll * 0.1}px, ${scroll * 0.5}px)`;
          if (hero) {
            hero.style.transform = `translate(-50%, calc(-50% + ${scroll * 0.7}px))`;
            hero.style.opacity = 1 - scroll / 700;
          }
          if (castle) castle.style.transform = `translateY(${scroll * 0.1}px) scale(${1 + scroll * 0.0004})`;
        }
        ticking = false;
      });
      ticking = true;
  ...
  document.querySelectorAll(".reveal, .reveal-card, .about-container").forEach((el, index) => {
  ...
  document.addEventListener("click", (e) => {
    ...
    const trigger = e.target.closest(".modal-trigger, .screenshot-btn, .view-cert-btn");
    
    if (trigger) {
      const rawData = trigger.getAttribute('data-img') || trigger.src;
      const title = trigger.closest(".project-info, .cert-info")?.querySelector("h3")?.innerText || trigger.alt;
  ...
  navLinks.forEach((link) => {
    link.addEventListener("click", () => {
      if (window.innerWidth < 992 && bootstrap.Collapse.getInstance(navCollapse)) {
